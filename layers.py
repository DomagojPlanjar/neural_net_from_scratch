...
class Layer(metaclass=ABCMeta):
    ...
    def backward_inputs(self, grads):
        """
            Perform the backward pass with respect to inputs.

            Parameters:
                grads (np.ndarray): Gradients of the loss with respect to the layer's outputs.

            Returns:
                np.ndarray: Gradients of the loss with respect to the layer's inputs.
        """
    ...
    def backward_params(self, grads):
        """
            Perform the backward pass with respect to parameters.

            Parameters:
                grads (np.ndarray): Gradients of the loss with respect to the layer's outputs.

            Returns:
                list: Gradients of the loss with respect to the layer's parameters.
        """
        pass
...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        output = inputs @ self.weights.transpose() + self.bias  # (N, num_outputs)
    ...
    def backward_params(self, grads):
        ...
        grad_weights = grads.transpose() @ self.inputs  # dim = (numt_outputs, num_inputs), same as self.weights
        grad_bias = np.sum(grads, axis=0)  # dim = (num_outputs, 1)
...
class ReLU(Layer):
    ...
    def backward_inputs(self, grads):
        ...
        Returns:
          ndarray of shape (N, C, H, W).
        """
        return np.multiply(grads, relu_derivative(self.input))


def stable_softmax(xmp):
    exp_x_shifted = np.exp(xmp - np.max(xmp))
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        """
        softmax_logits = np.array([stable_softmax(xmp) for xmp in x])
        tot_loss = -np.log(np.sum(softmax_logits * y, axis=1)) / x.shape[0]
    ...
    def backward_inputs(self, x, y):
        ...
        return (softmax_logits - y) / x.shape[0]
...
class L2Regularizer():
    ...
    def forward(self):
        return 0.5 * self.weight_decay * np.power(np.linalg.norm(self.weights), 2)
    ...
    def backward_params(self):
        ...
        """
        grad_weights = self.weight_decay * self.weights * 2
